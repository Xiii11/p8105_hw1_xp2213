...
The important variables are: `r names(penguins)`. Specifically, the dataset provides the demographic information about penguins, such as sex, year, bill depth and length, and flipper length and depth, and body mass. Also, island and species are also included. The mean flipper length `r mean(pull(penguins, flipper_length_mm), na.rm = TRUE)`
mm.
...
To further explore each variable in detail, there are 3 categories for variable "Island", which are: `r unique(pull(penguins,island))`. The species of sampled penguins are: `r unique(pull(penguins, species))`. 

The minimum, maximum and mean for bill length are `r min(pull(penguins_df, bill_length_mm))` mm, `r max(pull(penguins_df, bill_length_mm))` mm, and `r mean(pull(penguins_df, bill_length_mm))`, respectively.

The minimum, maximum and mean for bill depth are `r min(pull(penguins_df, bill_depth_mm))` mm, `r max(pull(penguins_df, bill_depth_mm))` mm, and `r mean(pull(penguins_df, bill_depth_mm))`, respectively.

The minimum, maximum and mean for flipper length are `r min(pull(penguins_df, flipper_length_mm))` mm, `r max(pull(penguins_df, flipper_length_mm))` mm, and `r mean(pull(penguins_df, flipper_length_mm))`, respectively.

The minimum, maximum and mean for flipper depth are `r min(pull(penguins_df, flipper_depth_mm))` mm, `r max(pull(penguins_df, flipper_depth_mm))` mm, and `r mean(pull(penguins_df, flipper_depth_mm))`, respectively.

The minimum, maximum and mean for body mass are `r min(pull(penguins_df, body_mass_g))` mm, `r max(pull(penguins_df, body_mass_g))` mm, and `r mean(pull(penguins_df, body_mass_g))`, respectively.
