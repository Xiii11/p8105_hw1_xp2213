...
To further explore each variable in detail, there are 3 categories for variable "Island", which are: `r unique(pull(penguins,island))`. The species of sampled penguins are: `r unique(pull(penguins, species))`. The minimum, maximum and mean values for bill length are `r min(pull(penguins_df, bill_length_mm))` mm, `r max(pull(penguins_df, bill_length_mm))` mm, and `r mean(pull(penguins_df, bill_length_mm))`, respectively. The minimum, maximum and mean values for bill depth are `r min(pull(penguins_df, bill_depth_mm))` mm, `r max(pull(penguins_df, bill_depth_mm))` mm, and `r mean(pull(penguins_df, bill_depth_mm))`, respectively. The minimum and maximum values for flipper length are `r min(pull(penguins_df, flipper_length_mm))` mm and `r max(pull(penguins_df, flipper_length_mm))` mm, respectively. The minimum, maximum and mean values for body mass are `r min(pull(penguins_df, body_mass_g))` mm, `r max(pull(penguins_df, body_mass_g))` mm, and `r mean(pull(penguins_df, body_mass_g))`, respectively.
